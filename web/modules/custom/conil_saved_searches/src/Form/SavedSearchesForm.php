...
class SavedSearchesForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Saved Searches.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Saved Searches.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.saved_searches.canonical', ['saved_searches' => $entity->id()]);
  }
